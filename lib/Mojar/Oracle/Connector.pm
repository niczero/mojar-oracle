package Mojar::Oracle::Connector;
use DBI 1.4.3;
use Mojo::Base 'DBI';
...
sub import {
  ...
  if (exists $param{-connector} and my $cname = delete $param{-connector}) {
    ...
    *{"${caller}::$cname"} = sub {
      my $self = shift;
      if (@_) {
        $self->{$cname} = (@_ > 1) ? Mojar::Oracle::Connector->new(@_) : shift;
        return $self;
      }
      return $self->{$cname} //= Mojar::Oracle::Connector->new;
    };
...
sub dsn {
  ...
  return (
    'DBI:'. $param->driver .q{:}
        . join(';', map sprintf(q{%s=%s}, $_, $param->$_), qw(host sid))
        . $cnf_txt,
    $param->user,
    $param->password,
    $dbd_param
...
sub schemata {
  ...
  my $schemata;
  eval {
    my $sql = q{SELECT * FROM dba_users};
...
# ============
package Mojar::Oracle::Connector::st;
@Mojar::Oracle::Connector::st::ISA = 'DBI::st';
...
Mojar::Oracle::Connector - Oracle connector (dbh producer) with added convenience

=head1 SYNOPSIS

In an application making only one type of connection.

  use Mojar::Oracle::Connector (
    cnfdir => '/var/local/auth/myapp',
    cnf => 'rw_localhost',
    schema => 'Users'
  );
  ...
  my $dbh = Mojar::Oracle::Connector->connect;
...
In an application making multiple types of connection.
  ...
  my $read_connector = Mojar::Oracle::Connector->new(
    cnf => 'ro_remotehost',
    schema => 'Orders'
  );
  my $write_connector = Mojar::Oracle::Connector->new(
    cnf => 'rw_localhost',
    schema => 'Reports'
  );
  ...
...
From the commandline.
  ...
  perl -MMojar::Oracle::Connector=cnf,ro_localhost,schema,Users,-dbh,1
    -E'say join qq{\n}, @{main->dbh->real_tables}'

=head1 DESCRIPTION

Oracle-specific extension (subclass) to L<DBI> in order to improve convenience,
...
=head2 C<new>
  ...
  Mojar::Oracle::Connector->new(label => 'cache', cnf => 'myuser_localhost');

Constructor for a connector, based on class defaults.  Takes a (possibly empty)
list of parameters.  Returns a connector (Mojar::Oracle::Connector object) the
...
addition to the passed parameters.

=head2 C<dsn>

  @dbi_args = Mojar::Oracle::Connector->dsn(
...
inconvenience of assembling sensible parameters yourself.
  ...
  my $dbh = DBI->connect(
    Mojar::Oracle::Connector->dsn(schema => 'foo', AutoCommit => 0)
  );

=head2 C<dsn_to_dump>

  warn(Mojar::Oracle::Connector->dsn_to_dump(@dsn));
...
=head2 C<Defaults>
  ...
  say Mojar::Util::dumper(Mojar::Oracle::Connector->Defaults);
...
(possibly empty) parameter hash.  Returns a connector (Mojar::Oracle::Connector
...
you need undef as the second argument.
  ...
  printf '%s can: %s', $_->{User}, $_->{Priv}
    for $dbh->selectall_arrayref_hashrefs(
      q{SELECT * FROM user WHERE User != ?}, undef, 'DBA'
    );
...
This class is a fork of L<Mojar::Mysql::Connector>.  After some refactoring of
that class it became clear much of its code was useful to my Oracle
connections.  The two will stay separate because of the many differences
(handling of UTF-8, .cnf files, etc).
